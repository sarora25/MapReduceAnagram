...
# Author: Swati Arora (Modified the open source sample application to include implementation for Anagram)
# Description: Gives frequency of unique anagram in the input files#
# Date: 12/18/2013 #
...
class FileMetadata(db.Model):
  ...
  anagram_link = db.StringProperty()
...
class IndexHandler(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    elif self.request.get("anagram"):
      pipeline = AnagramPipeline(filekey, blob_key)
...
def word_count_reduce(key, values):
  ...
  yield "%s: %s: %d\n" % (key, list(values), len(values))

def anagram_map(data):
  """Anagram count map function."""
  (entry, text_fn) = data
  text = text_fn()
  ...
  for s in split_into_sentences(text):
    for w in split_into_words(s.lower()):
      ...
      wordchars = list(word)
      wordchars.sort()
      sortedword = ''.join(wordchars) 
    
      yield (sortedword,w)


def anagram_reduce(key, values):
  """Anagram count reduce function."""
  count = len(values)
  yield "%s: %s\n" % (key, count)
...
class AnagramPipeline(base_handler.PipelineBase):
  """A pipeline to anagram demo.
  ...
  def run(self, filekey, blobkey):
    ...
    output = yield mapreduce_pipeline.MapreducePipeline(
        ...
        shards=16)
    yield StoreOutput("Anagram", filekey, output)
...
class StoreOutput(base_handler.PipelineBase):
  ...
  def run(self, mr_type, encoded_key, output):
    ...
    elif mr_type == "Anagram":
      m.anagram_link = output[0]
    elif mr_type == "Index":
